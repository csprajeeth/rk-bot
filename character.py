...
class Character():
    ...
    def show_inbox(self):
        ...
        for mail in mails:
            ...
            if a != None:
                mail.a.extract()
                self.logger.write(a.text.encode('ascii','ignore') + " [" + mail.text.encode('ascii', 'ignore').strip() + "]\n")
            else:
    ...
    def create_group(self, group_type = "simple", size = 5, admission="free"):
        """
        Create a group
        """

        group_type = 'arme' if group_type == "armed" else 'simple'
        admission = 'ouvert' if admission == "free" else 'demande'
        self.visit(game_url+"Action.php?action=86", urllib.urlencode({'Type':group_type,
                                                                      'NombreMax':str(size), 'Admission':admission}))
